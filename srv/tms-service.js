...
module.exports = cds.service.impl(async function () {
    ...
    this.on('updateBillOfLading', async req => {
        try {
            ...
            const retData = await executeHttpRequest(
                ...
                {
                    method: 'patch',
                    url: `/sap/opu/odata/sap/API_OUTBOUND_DELIVERY_SRV;v=0002/A_OutbDeliveryHeader('${deliveryDetails.deliveryID}')`,
                    data: data,
                    headers: {
                        'If-Match': '*'
                    }
            ...
            if (retData.status === 204) {
                await tx.run(UPDATE(Deliveries).set({
                    shipmentStatus: 'Tracking Assigned',
                    legType: 'Road',
                    trackingNumber: trackingNumber
                }).where({ ID: req.params[0].ID }))
                return req.notify(200, `Tracking Number ${trackingNumber} updated for Delivery ${deliveryDetails.deliveryID}`)
            }
